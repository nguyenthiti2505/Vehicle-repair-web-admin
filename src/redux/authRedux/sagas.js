import { put, takeLatest } from 'redux-saga/effects'
import * as Types from './types'
import { callApi } from '../../utils/apiCaller'

function* fetchAuthSaga({ payload }) {
  try {
    const { pageIndex, pageSize, fromDate, toDate } = payload
    const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..xbHaI2XcaoHPDSh24NrJwe_c9WY9Cv8ZLJA7j6CNsKA"
    
    let url = `dashboard/auth?offset=${pageIndex || 1}&limit=${pageSize || 10}`
    if (fromDate && toDate) {
      url += `&fromDate=${fromDate}&toDate${toDate}`
      
    }
    const response = yield callApi(url, 'GET', null, token)
    yield put({ type: Types.FETCH_AUTH_SUCCEEDED, payload: response.data })
  } catch (error) {
    console.log("function*fetchAuthSaga -> error", error)
    yield put({ type: Types.FETCH_AUTH_FAILED, payload: {} })
  }
}

export const watchAuthSaga = [
  takeLatest(Types.FETCH_AUTH_REQUEST, fetchAuthSaga)
]